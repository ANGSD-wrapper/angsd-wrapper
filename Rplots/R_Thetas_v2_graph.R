...
plot.thetas <- function(estTheta) {
  #   Watterson's Theta
  hist(estTheta$Watterson, right = FALSE, xlab = "Watterson's Theta",
       main = "Frequency Distribution of Watterson's Theta", col = "lightsteelblue")

  plot(x = estTheta$WinCenter, y = estTheta$Watterson, type = "p", xlab = "Position (bp)",
  ...
  barplot(estTheta$Watterson, col = sample(x= colors(distinct = TRUE),
                                   size = length(estTheta$Watterson), replace = FALSE),
          main = "Estimated Watterson's Theta per base pair",
          xlab = "Region", ylab = "Estimated Theta", las = 2, names.arg = FALSE)

  #   Pairwise Theta
  hist(estTheta$Pairwise, right = FALSE, xlab = "Pairwise Theta",
       main = "Frequency Distribution of Pairwise Theta", col = "seagreen2")

  plot(x = estTheta$WinCenter, y = estTheta$Pairwise, type = "p", xlab = "Position (bp)",
       ylab = "Pairwise Theta", main = "Estimators of Pairwise Theta", col = "seagreen2", pch = 19)

  barplot(estTheta$Pairwise, col = sample(x= colors(distinct = TRUE), size = length(estTheta$Pairwise), replace = FALSE), main = "Estimated Pairwise Theta per base pair", xlab = "Region", ylab = "Estimated Theta", las = 2, names.arg = FALSE)

  #   Tajima's D
  hist(estTheta$Tajima, right = FALSE, xlab = "Tajima's D",
       main = "Frequency Distribution of Tajima's D", col = "lightsalmon")

  plot(x = estTheta$WinCenter, y = estTheta$Tajima, type = "p", xlab = "Position (bp)",
       ylab = "Tajima's D", main = "Estimators of Tajima's D",
       col = "lightsalmon", pch = 19)

  barplot(estTheta$Tajima, col = sample(x= colors(distinct = TRUE), size = length(estTheta$Tajima), replace = FALSE), main = "Estimated Tajima's D per base pair", xlab = "Region", ylab = "Estimated Tajima's D", las = 2, names.arg = FALSE)
}
...
main <- function() {
  ...
  estTheta <- data.frame(Watterson = wattersons, Pairwise = pairwises, Tajima = pest$Tajima, WinCenter = pest$WinCenter)
  #return(wattersons, pairwises, estTheta)
  pdf(file = outputNamem, width = 6, height = 6)
  options(scipen = 5)
  plot.thetas(estTheta = estTheta)
  dev.off()
}
